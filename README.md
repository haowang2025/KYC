# triage — 企业级 KYC/AML 合规初筛智能体（AI CAN DO IT 参赛作品）

triage 是一个面向 B2B 交易/商户准入（KYC/AML）的**风险初筛 Agent**：输入自然语言或结构化信息，输出 **GREEN / YELLOW / RED** 决策，并给出 3 条原因、3 个下一步动作与证据摘要。
...
triage 将“合规初筛”变成一个可复用的 API + UI 能力，用于内部系统接入与批量处理。
...
# 1) 启动 Web（UI + API）
python -m triage --serve --port 8000

# 2) CLI 一次性检查
python -m triage --name "Parsian Bank" --country "Iran"
```
...
docker run --rm -p 8000:8000 --env-file .env triage
```

## 目录结构

- `triage/`：核心后端（API + CLI + 规则引擎 + KG 客户端）
